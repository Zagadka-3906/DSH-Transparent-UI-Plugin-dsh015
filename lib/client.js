window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// DSH 0.1.7 renamed the 16px check icon (IconCheckOutline16 ->
		// IconCheckOutlineMedium/Regular); alias the old name back so the same
		// bundle runs on both 0.1.5 and 0.1.7.
		if (!_deepseek_ai_dsh_client_ui_primitives.IconCheckOutline16) {
			_deepseek_ai_dsh_client_ui_primitives = Object.assign({}, _deepseek_ai_dsh_client_ui_primitives, { IconCheckOutline16: _deepseek_ai_dsh_client_ui_primitives.IconCheckOutlineMedium || _deepseek_ai_dsh_client_ui_primitives.IconCheckOutlineRegular });
		}
		...
		*/
		function apply(ctx) {
			...
			const layer = new AquaLayer(ctx);
				...
				const masterStore = createAquaRowStore();
				let pluginBound;
				let appearanceBound;
				let masterBound;
				let revision = 0;
			const payload = () => {
				...
				const sync = () => {
					...
					masterBound?.sync(next, revision);
					revision += 1;
				};
